    ...
    while True:
        if wait_for_start_signal():
            ...
            try:
                ...
                train_images, train_annotations = proxy.get_human_data(f"client3")
                ...
                model_name = f"client3_model.h5"
                model.save(model_name)

                # Send the model to the coordinator
                print("Sending model to coordinator...")
                with open(model_name, "rb") as model_file:
                    proxy.upload_model(xmlrpc.client.Binary(model_file.read()), f"client3")
