...
def get_cifar(client_id):
    ...
    if client_id not in client_ranks:
        return [], []

    proportion = client_ranks[client_id]
    total_data_size = len(shuffled_data_cifar)

    # Get the start index based on the sum of proportions for clients before the current one
    sorted_clients = sorted(client_ranks.keys(), key=lambda x: client_ranks[x])
    start_index = 0
    for c_id in sorted_clients:
        if c_id == client_id:
            break
        start_index += int(client_ranks[c_id] * total_data_size)
