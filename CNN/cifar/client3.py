...
CLIENT_PORT = 9093  # Unique port for each client. Replace X with the client number.
...
if __name__ == "__main__":
    ...
    while True:
        if wait_for_start_signal():
            ...
            data, target = proxy.get_cifar(f"client3")  # Replace X with the client number
            ...
            model_name = f"client3_model.h5"  # Replace X with the client number
            model.save(model_name)

            # Send the model back to the coordinator
            with open(model_name, "rb") as model_file:
                proxy.upload_model(xmlrpc.client.Binary(model_file.read()), f"client3")  # Replace X with the client number
