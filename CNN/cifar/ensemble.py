...
def ensemble_and_evaluate_model_mixture():
    ...
    # Print evaluation metrics
    print(f"model_mixture_Accuracy: {accuracy_score(y_test, final_label_predictions)}")
...
def test_ensemble_methods():
    # Load the client models
    all_models = []
    for client_id in ["client1", "client2", "client3"]: # You can modify this list to include all client ids you have models for
        model_name = f"{client_id}_model.h5"
        if os.path.exists(model_name):
            all_models.append(load_model(model_name))
        else:
    ...
    # Test the ensemble methods here
    # ensemble_and_evaluate_voting()
    ensemble_and_evaluate_avg()
    ensemble_and_evaluate_stacking()
    ensemble_and_evaluate_bagging()
